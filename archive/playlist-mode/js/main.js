...
(async () => {
  ...
  try {
    const { audioInput, trackUrls, stickerUrls, backgroundUrls, virguleUrls, phaseVideoUrls } =
      await loadFromServer();
    audio.setAudioInputMode(audioInput);
    if (audio.isMicrophoneMode()) {
      document.body.classList.add('ssi-micro-audio');
      window.addEventListener('beforeunload', () => audio.stopMicrophoneAnalysis());
    }
    ...
    if (trackUrls.length) {
      debugLog(
        ...
        trackUrls.map((u, i) => `[${i}] ${decodeURIComponent((u.split('/').pop() || u).replace(/\+/g, ' '))}`).join(' · '),
      );
    }
    ...
    debugLog('Init terminée — playlist:', audio.getPlaylistLength(), '| jingleUrls:', audio.getJingleUrls().length);
    ...
    if (audio.isMicrophoneMode()) {
      ...
      let warmReady = false;
      getBrowserWarmPromise().then(() => { warmReady = true; });

      const startOnFirstClick = (e) => {
        if (e.target.closest && e.target.closest('.mini-controls-hud .mini-controls')) return;
        ...
        if (!warmReady) {
          ...
          getBrowserWarmPromise().then(() => {
            ...
            if (span && span.parentElement && !span.parentElement.hidden) {
              span.textContent = '▶ cliquez pour démarrer — la caméra peut être demandée ici aussi';
            }
          });
          return;
        }
        ...
        if (!audio.getCurrentAudio()) {
          ...
          if (ju.length) {
            ...
            debugLog('Démarrage — virgule intro puis 1ᵉʳ morceau du tour (index playlist', firstIdx, ')');
