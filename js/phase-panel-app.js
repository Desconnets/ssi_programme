...
async function bootstrap() {
  ...
  const videoSelect = /** @type {HTMLSelectElement} */ (
    document.getElementById("panelVideoSelect")
  );
  const bgSelect = /** @type {HTMLSelectElement} */ (
    document.getElementById("panelBgSelect")
  );
  const bgOpacity = /** @type {HTMLInputElement} */ (
    document.getElementById("panelBgOpacity")
  );
  const bgOpacityVal = document.getElementById("panelBgOpacityVal");
  const bgAuto = /** @type {HTMLInputElement} */ (
    document.getElementById("panelBgAutoRotate")
  );
  ...
  const idleResumeSec = /** @type {HTMLInputElement} */ (
    document.getElementById("panelIdleResumeSec")
  );
  ...
  const textInputElement = /** @type {HTMLInputElement | null} */ (
    document.getElementById("panelTextInput")
  );
  ...
  const runPhase = async (p) => {
    ...
    if (p.id === "text") body.textContent = wysiContent || textInputElement?.value || "";
    log.append(
      "cmd",
      `${p.label} (${p.id})`,
      vi != null ? `vidéo #${vi}` : (body.textContent ?? ""),
    );
    try {
      ...
      if (res.ok && res.json) {
        log.append(
          "ok",
          `serveur seq=${res.json.seq}`,
          String(res.json.phase || ""),
        );
  ...
  const refresh = async () => {
    try {
      ...
      const phases =
        Array.isArray(j.panelPhases) && j.panelPhases.length > 0
          ? j.panelPhases
          : FALLBACK_PANEL_PHASES;
      ...
      if (j.bgGradientOpacity == null || j.bgGradientOpacity === "") {
        ...
        const pct = Math.round(
          Math.max(0, Math.min(1, Number(j.bgGradientOpacity))) * 100,
        );
      ...
      btnPausePhases.textContent = isPaused
        ? "▶ Reprendre les phases"
        : "⏸ Pause phases (fond uniquement)";
      btnPausePhases.style.background = isPaused ? "#2a6e2a" : "";

      bgAuto.checked = Boolean(j.backgroundAutoRotate);
      const nBg = bgFiles.length;
      if (
        !j.backgroundAutoRotate &&
        j.backgroundVideoIndex != null &&
        nBg > 0
      ) {
        const want = Math.max(
          0,
          Math.min(nBg - 1, Math.floor(Number(j.backgroundVideoIndex))),
        );
  ...
  btnBgOpacityApply.addEventListener("click", async () => {
    const v =
      Math.max(0, Math.min(100, parseInt(bgOpacity.value, 10) || 0)) / 100;
  ...
  btnPausePhases.addEventListener("click", async () => {
    ...
    try {
      ...
      if (res.ok && res.json) {
        ...
        btnPausePhases.textContent = p
          ? "▶ Reprendre les phases"
          : "⏸ Pause phases (fond uniquement)";
        btnPausePhases.style.background = p ? "#2a6e2a" : "";
        log.append(
          "ok",
          p ? "Phases en pause — fond actif" : "Phases reprises",
        );
